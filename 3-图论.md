...
int main(int argc, char const *argv[])
{
    ...
    for (int kk=0; kk<t; ++kk) {
        ...
        for (int i=1; i<=n; ++i) {
            for (int j=1; j<n; ++j) {
                if (g[i][j] != johnson::INF) printf("%lld ", g[i][j]);
                else printf("# ");
            }
            if (g[i][n] != johnson::INF) printf("%lld\n", g[i][n]);
            else puts("#");
        }
...
namespace dinic
{
    struct E
    {
        int to;
        LL cp;
        E(int to=0, int cp=0):to(to), cp(cp){}
    };
    struct Dinic {
...
struct extdinic1
{
    dinic::Dinic dc;
    int d[N], S, T, n;
    void addedge(int u, int v, int minw, int maxw, int eid)
    {
        dc.addedge(S, v, minw);
        dc.addedge(u, T, minw);
        id[eid] = dc.ecnt;
        dc.addedge(u, v, maxw - minw);
    }
    void init(int n)
    {
        S = n + 1, T = n + 2;
...
namespace treeEquiv {
    ...
    void dfs1(int u, int fa) {
        ...
        for (int v:G[u]) {
            if (v == fa) continue;
            LL tmp = (Hash[u] - (ULL)sz[u]*P - 1) ^ (Hash[v] + Base);
            tmp += ((ULL)n-sz[v])*P + 1;
            Hash[v] -=  (ULL)sz[v]*P+1;
            Hash[v] ^= (tmp + Base);
            Hash[v] += (ULL)n*P+1;
    ...
    int cnt(int _n) {
        ...
        memset(Hash, 0, sizeof(ULL)*(n+1));
        memset(root, 0, sizeof(ULL)*(n+1));
        ...
        int sz = unique(root+1, root+n+1) - root-1;
        return sz;
    }
}
```
