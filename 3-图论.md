...
* $\deg v = \sum\limits_{w\in V}a_{vw}$
* $D(G) = diag(\deg v : v \in V)$
* $L(G) = D(G) − A(G)$
* let $a(T)$ be the product of the edgeweights of $T$ . For any vertices $v$ and $w$, the (weighted) number of spanning trees of $G$ is
