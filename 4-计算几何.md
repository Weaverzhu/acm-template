...
struct P {
    ...
    static void setSrc(const P &s) {
        src = s;
    }

    static bool cmp(const P &a, const P &b) {
        if (a == src) return 1;
        else if (b == src) return 0;
        return (a-src) * (b-src) >= 0;
    }

    LD dist(const P &other) const {
        LD dx = x-other.x, dy = y-other.y;
        return sqrt(dx*dx+dy*dy);
    }
};
P P::src;
int Convex(P a[], int n, P s[]) {
    int top = 0;
    P::src = a[0];
    for (int i=1; i<n; ++i) {
        if (a[i].y < P::src.y) P::src = a[i];
    }
    sort(a, a+n, P::cmp);
    s[top++] = a[0];
    s[top++] = a[1];
    s[top++] = a[2];
    for (int i=3; i<n; ++i) {
        while (top >= 3 && sgn((a[i] - s[top-2]) * (s[top-1] - s[top-2])) >= 0) --top;
        s[top++] = a[i];
    }
    return top;
}
