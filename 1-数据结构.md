...
struct sg {
    ...
    static const int NULLV = 1e9, Nmin = 1e9, Nmax = -1e9, N = 5e5+5;

    struct Tag {
        V setv, addv;
        Tag(V setv = NULLV, V addv = 0) :setv(setv), addv(addv){}
        void copy(const Tag &fa) {
            if (setv == NULLV && addv == 0 || fa.setv != NULLV) *this = fa;
            else {
                if (setv != NULLV) setv += fa.addv;
                else {
                    addv += fa.addv;
                }
            }
        ...
        bool empty() {
            return setv == NULLV && addv == 0;
        }
    };

    struct P {
        LL sumv, minv, maxv;
        P(V sumv=0, V minv = Nmin, V maxv = Nmax) : sumv(sumv), minv(minv), maxv(maxv) {}

        void copy(const Tag &fa, int l, int r) {
            if (fa.setv != NULLV) {
                minv = maxv = fa.setv;
                sumv = fa.setv * (r-l+1);
            } else {
    ...
    void up(int rt) {
        dat[rt].minv = min(dat[lson].minv, dat[rson].minv);
        dat[rt].maxv = max(dat[lson].maxv, dat[rson].maxv);
        dat[rt].sumv = dat[lson].sumv + dat[rson].sumv;
    }

    void down(int rt, int l, int r) {
        if (lazy[rt].empty()) return;
        lazy[lson].copy(lazy[rt]);
        lazy[rson].copy(lazy[rt]);
        int m = (l+r) >> 1;
        dat[lson].copy(lazy[rt], ls);
        dat[rson].copy(lazy[rt], rs);
        lazy[rt] = Tag();
    }
    ...
    void init(int n, int val[]) {
        ...
        FOR (i, 1, n+1) {
            ...
            dat[nn+i-1] = P(val[i], val[i], val[i]);
            dbg(i, nn+i-1, dat[nn+i-1].sumv, val[i]);
        }
        FOR (i, nn+n, 2*nn) {
            dbg(i);
            dat[i] = P();
        }
        FOR (i, 1, 2*nn) lazy[i] = Tag();
        FORD (i, nn-1, 0) up(i);
    }

    void setu(int rt, int L, int R, V v,int l, int r) {
        if (R < l || r < L) return;
        if (L <= l && r <= R) {
            dat[rt] = P(v*(r-l+1), v, v);
            lazy[rt] = Tag(v, 0);
    ...
    void addu(int rt, int L, int R, V v, int l, int r) {
        if (R < l || r < L) return;
        if (L <= l && r <= R) {
            if (lazy[rt].setv != NULLV) {
                lazy[rt].setv += v;
            } else {
                lazy[rt].addv += v;
            }
            dbg(l, r, v, dat[rt].sumv, rt);
            dat[rt].sumv += v*(r-l+1);
            dat[rt].minv += v;
            dat[rt].maxv += v;
             dbg(l, r, v, dat[rt].sumv);
            return;
        }
        down(rt, l, r);
        int m = (l+r) >> 1;
    ...
    P q(int rt, int L, int R, int l, int r) {
        if (R < l || r < L) return P();
        if (L <= l && r <= R)
            return dat[rt];
        int m = (l+r) >> 1;
        down(rt, l, r);
        P p1 = q(lson, L, R, ls), p2 = q(rson, L, R, rs);
        return P(p1.sumv+p2.sumv, min(p1.minv, p2.minv), max(p1.maxv, p2.maxv));
    }
} it;
```
