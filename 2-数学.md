...
struct P {
    ...
    void mul(const P &other) {
        int len = 1, lim = max(n, other.n);
        while (len < 2*lim) len <<= 1;
        NTT::NTT_init(len);
        for (int i=0; i<other.n; ++i) {
            cc[i] = other.v[i];
        }
        for (int i=other.n; i<len; ++i) {
            cc[i] = 0;
        }
        NTT::NTT(v, len, 1);
        NTT::NTT(cc, len, 1);
...
A \times B^{'} \equiv 1 ( \mod x^{n/2}) \\
A \times (B - B^{'}) \equiv 0( \mod x^{n/2}) \\
B - B^{'} \equiv 0( \mod x^{n/2}) \\
(B - B^{'})^2 \equiv 0 ( \mod x^n) \\
...
```cpp
P B[20], ta, tb, A;
void copyP(P &dst, P &src) {
    dst.n = src.n;
    memcpy(dst.v, src.v, sizeof(LL) * src.n);
}
inline void clr20(LL v[], int l, int r, int val) {
    if (l>r) return;
    memset(v+l, val, sizeof(LL)*(r-l));
}
void work(int n) {
    if (n == 0) {
        B[0].n = 1;
        B[0].v[0] = get_inv(A.v[0], MOD);
    } else {
        work(n-1);
        copyP(ta, A);
        copyP(tb, B[n-1]);
        int len=max(ta.n, tb.n), l=1;
        while (l<2*len) l <<= 1;
        clr20(ta.v, ta.n, l, 0);
        clr20(tb.v, tb.n, l, 0);
        NTT::NTT_init(l);
        ...
        NTT::NTT(tb.v, l, 1);
        for (int i=0; i<l; ++i) {
            ta.v[i] = (2 + MOD -ta.v[i]*tb.v[i]%MOD)%MOD * tb.v[i]%MOD;
        }
        NTT::NTT(ta.v, l, -1);
        ta.n = 1<<n;
        copyP(B[n], ta);
